;(function($){
  ...
  var xSlider = function(el, userConfig) {
    ...
    for (var i = 0; i < slider_img_length  ; i++) {
    ...
    // 初始化默认显示图片位置
    slider_img_ul.css('left', - this.config.w * this.config.current  )
    ...
    slider_btn_right.on('click', function(event) {
      ...
      if(_this.config.current < slider_img_length  ){
        toggleInterval ()
        _this.config.current ++
        if(_this.config.current != slider_img_length  ) {
          slider_img_ul.stop(true, false).animate({left: - _this.config.w * _this.config.current }, _this.config.speed, function () {
      ...
      if(_this.config.current > -1){
        ...
        if(_this.config.current === -1){
          slider_img_ul.stop(true, false).animate({left: 0}, _this.config.speed, function() {
            slider_img_ul.css('left', - _this.config.w * (slider_img_length  ))
    ...
    slider_dot_ul_li.on('click', function(event) {
      ...
      active(index)
      slider_img_ul.stop(true, false).animate({left: - _this.config.w * index}, _this.config.speed, function() {
